...
- 删除，需要传入2个参数：要删除的第一个元素的位置和要删除的元素数量，例如 splice(0, 2) 删除数组中的前两个元素
- 插入，需要传入3个参数：开始位置、要删除的元素数量（可以是0）、要插入的元素，例如 splice(2, 0, "red", "green") 会从数组位置2开始插入字符串"red"和"green"
- 替换，在删除元素的同时插入新元素，需要传入3个参数：开始位置、要删除的元素数量、要插入的元素，要插入的元素数量不一定跟删除的元素数量一致，例如 splice(2, 1, "red", "green") 会在位置2删除一个元素，然后插入字符串"red"和"green"
